...
class datas_parse:
	...
	def time_get(date, length, head, sheet, row_num):   # Mtg時間を取得
		for col_num in range(length):
			if head[col_num].count(date):
				st,en,rest = datas_parse.parse_time(col_num+1,sheet,row_num)
				return (st,en,rest)
	...
	def parse_time(col_num, sheet, row_num):
		...
		if record[row_num] == '':												# 欠席
			...
			st,en,rest = datas_parse.time_extraction(record[record_len-1])		# 最初から最後まで出席
			return (st,en,rest)
		else:
			st,en,rest = datas_parse.attendance_and_rest_time(record[row_num],record[record_len-1])	# 遅刻、早退有り
			return (st,en,rest)
	...
	def time_extraction(time_record):
		...
		if index_kyu == -1:
			print('[-] not include 休')
			quit()
		st = time_record[0:index_kyu-6]
		en = time_record[index_kyu-5:index_kyu]
		index_h = time_record.find('h')
		if index_h == -1 or index_h < index_kyu:
			print('[-] not include h or reverse')
			quit()
		rest = time_record[index_kyu+2:index_h]
		return (st,en,rest)
	...
	def attendance_and_rest_time(record,time_record):
		...
		st,en,rest = datas_parse.time_extraction(time_record)

		if record.count('早'):					# 早退パターン
			en = record[index_wave+1:index_wave+6]
			if index_kyu > 0:
				rest = record[index_kyu+2:index_h]
		elif 5 < index_kyu-index_wave:			# oo:oo~oo:ooパターン
			st = record[index_wave-5:index_wave]
			en = record[index_wave+1:index_wave+6]
			rest = record[index_kyu+2:index_h]
